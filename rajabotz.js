...
owner = setting.6282188585604
botname = setting.Raja BOTz
zerokey = setting.ZeroKey
ownername = setting.Raja × Hosting

//━━━━━━━━━━━━━━━[ MODUL EXPORTS ]━━━━━━━━━━━━━━━━━//

module.exports = rajabotz = async (rajabotz, mek, _welkom) => {
	try {
		...
		let botNumber = RajaBOTz.user.jid
		let ownerNumber = [`${owner}@s.whatsapp.net`, `6282188585604@s.whatsapp.net`]
		...
		let conts = mek.key.fromMe ? rajabotz.user.jid : rajabotz.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        let pushname = mek.key.fromMe ? rajabotz.user.name : conts.notify || conts.vname || conts.name || '-'
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			...
			success: 'Done Jangan Lupa Subscribe Raja × Hosting',
			error: {
        ...
        let fakegroup = (teks) => {
            zeroyt7.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
                        participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6282188585604-1604595598@g.us" } : {})
                    },
                    message: {
                        "imageMessage": {
                            "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc",
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `SUBSCRIBE Raja × Hosting`, 
                            orderTitle: `SUBSCRIBE Raja × Hosting`,
                            thumbnail: raja, //Gambarnye
...
setTimeout(() => {
rajabotz.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
}, 0)
}
...
╭─❒ 「 Bot Info 」 ❒
├ Nama Bot : ${Raja BOTz}
├ Nama Owner : ${Raja × Hostimg}
├ Prefix : Multi Prefix
├ Nomor Owner : ${owner.split('@')[082188585604]}
...
└❏
╭─❒ 「 User Info 」 ❒
├ Status : ${isOwner ? 'Owner' : 'Raja BOTz'}
├ Nama User : ${pushname}
...
timestamp = speed();
				latensi = speed() - timestamp
                  	teks =
`┏━➤ *INFO BOT* 
*┃┃* Creator Bot : Raja Botz
*┃┃* Nama Owner : ${Raja × Hosting}
*┃┃* Nama Bot : ${Raja BOTz}
*┃┃* Prefix : Multi Prefix
...
*┗━━━━━━━ •*`
        ...
        sendButton(from, teks, '©Created : Raja × Hosting', but, mek)
break
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Jangan Lupa Subscribe Raja × Hosting`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
zeroyt7.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Jangan Lupa Subscribe Raja × Hosting' })
fs.unlinkSync(ran)
})
break
...
let vname = rajabots.contacts[i] != undefined ? rajabotz.contacts[i].vname || rajabotz.contacts[i].notify : undefined
inilist.push({
"displayName": 'ZeroYT7',
"vcard": 'BEGIN:VCARD\n'
+ 'VERSION:3.0\n'
...
await rajabotz.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
}
...
raja × hosting.sendMessage(`6282188585604@s.whatsapp.net`, options, text, { quoted: ftrol })
...
ssd = child.replace(/Memory: 64 Gb /, "Ram: 4 Gb");
...
if (isOwner) {
    ...
    e = String(e)
    if (!e.includes("this.isRaja") && !e.includes("jid")) {
